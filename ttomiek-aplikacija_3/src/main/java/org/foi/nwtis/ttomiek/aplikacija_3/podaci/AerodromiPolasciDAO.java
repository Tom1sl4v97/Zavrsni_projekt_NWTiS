...
public class AerodromiPolasciDAO {
	 ...
	 */
	public List<AvionLeti> dohvatiSveAerodromePolaska(Connection veza, String icao, String vrijemeOd, String vrijemeDo,
			int vrsta) {
		long datum1;
		long datum2;
		...
		String upit = "SELECT * FROM AERODROMI_POLASCI WHERE ESTDEPARTUREAIRPORT = \"" + icao + "\" AND FIRSTSEEN >= "
				+ datum1 + " AND FIRSTSEEN <= " + datum2 + " ORDER BY FIRSTSEEN;";

		List<AvionLeti> sviPolasci = dohvatiPolaskeSaBaze(veza, upit);
		return sviPolasci;
	}
	...
	/**
	 * Metoda dohvaća sve aerodrome polaska.
	 *
	 * @param veza parametar veze
	 * @param upit string vrijednost za upit koji se provodi nad bazom
	 * @return vraća listu dohvaćenih aerodroma polaska, inače null
	 */
	private List<AvionLeti> dohvatiPolaskeSaBaze(Connection veza, String upit) {
